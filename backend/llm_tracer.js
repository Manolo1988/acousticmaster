...
const safeWrite = (filePath, content) => {
  try {
    ...
    console.warn(`[llm_tracer] ⚠️ 写入失败: ${filePath} — ${e.message}`);
  }
};
...
export function traceLlmCall({ stage, planTitle, prompt, response, simulationContext, error, metadata }) {
  ...
  if (prompt) {
    safeWrite(`${TRACE_DIR}/${prefix}-prompt.txt`, String(prompt));
  }
  if (response) {
    safeWrite(`${TRACE_DIR}/${prefix}-response.md`, String(response));
  }
  if (simulationContext && typeof simulationContext === "object") {
    safeWrite(`${TRACE_DIR}/${prefix}-sim-context.json`, JSON.stringify({
    ...
    safeWrite(`${TRACE_DIR}/${prefix}-sim-context.json`,
      JSON.stringify({ status: "null_or_missing", note: "simulationContext 为空 — 仿真章节不会生成" }, null, 2));
  }
  if (metadata || planTitle) {
    safeWrite(`${TRACE_DIR}/${prefix}-meta.json`,
      JSON.stringify({ planTitle: label, stage, timestamp: now, ...(metadata || {}) }, null, 2));
  }
  if (error) {
    safeWrite(`${TRACE_DIR}/${prefix}-error.txt`, String(error.message || error));
  }
  ...
  if (prompt && response) {
    ...
    safeWrite(LATEST_CONVERSATION_FILE, conversationMd);
  }
}
...
export function traceFinalReport(planTitle, markdownRaw, markdownProcessed, simulationChapter) {
  ...
  // 归档
  safeWrite(`${TRACE_DIR}/${sessionId}-final-raw.md`, String(markdownRaw || ""));
  safeWrite(`${TRACE_DIR}/${sessionId}-final-processed.md`, String(markdownProcessed || ""));
  if (simulationChapter) {
    safeWrite(`${TRACE_DIR}/${sessionId}-final-simulation-chapter.md`, String(simulationChapter));
  }
  safeWrite(`${TRACE_DIR}/${sessionId}-final-meta.json`, JSON.stringify({
  ...
  // 也保存到项目根目录
  safeWrite(`${PROJECT_ROOT}/最新一次方案报告_最终版.md`, String(markdownProcessed || ""));
}
